...
genome <- args[7]
work_dir <- args[8]
#set genome to be used for gene and genome annotations to be mouse mm10 or human hg38
...
if (nchar(conditionA) > 1) {
  subsetA <- which(proj_filter$Condition == conditionA & proj_filter$Clusters %in% clusterA_list, )
...
if (nchar(conditionB) > 1) {
  subsetB <- which(proj_filter$Condition == conditionB & proj_filter$Clusters %in% clusterB_list, )
...
df <- data.frame(project_select@cellColData) %>%
  mutate(UpdateClustName = ifelse(Clusters %in% clusterB_list, conditionB, ifelse(Clusters %in% clusterA_list, conditionA, Clusters)))
...
write.csv(SampleGeneList,file=paste0(project_name,"_Sample_gene_list.csv"),row.names = FALSE)
...
pairwise_df$Significance = ifelse(pairwise_df$pvalue < 0.05 & abs(pairwise_df$log2FC) >= 0.4, 
                                  ifelse(pairwise_df$log2FC> 0 ,colnames(assay(Select_genes))[2],colnames(assay(Select_genes))[1]),
                                  'Not significant')
write.csv(pairwise_df,file=paste0(project_name,"_gene_markers.csv"),row.names = FALSE)

volcano<- EnhancedVolcano(pairwise_df,
                lab =pairwise_df$pairwise_Genes ,
                x = 'log2FC',
                y = 'pvalue',
                ylim=c(0,abs(min(log10(pairwise_df$pvalue)))),
                xlim= c(-2.5,2.5),
                title = paste0(colnames(assay(Select_genes))[2],' vs ',colnames(assay(Select_genes))[1]),
                pCutoff = 0.05,
                FCcutoff = 0.4,
                pointSize = 1.0,
                labSize = 4.0)
...
print(volcano)  # This will print the plot to the PDF device
...
pma <- plotMarkers(seMarker = markerTest, name = conditionA, cutOff = "FDR <= 0.1 & abs(Log2FC) >= 1", plotAs = "Volcano")
#pma <- markerPlot(seMarker = markerTest, name = conditionA, cutOff = "FDR <= 0.1", plotAs = "MA")
...
#markerList <- getMarkers(markerTest, cutOff = "FDR <= 0.1")

#Collect data with annotations
peak_data = data.frame(project_select@peakSet@ranges,project_select@peakSet@elementMetadata)
total <- merge(peak_data,markerList,by=c("start","end"))

write.csv(total,file=paste0(project_name,"_peak_markers.csv"),row.names = FALSE)

#project_select <- addMotifAnnotations(ArchRProj = project_select, motifSet = "cisbp", name = "Motif")
...
print(ggUp)  # This will print the plot to the PDF device
dev.off()   # Close the PDF device
...
write.csv(df2,file=paste0(project_name,"_motifsdown.csv"),row.names = FALSE)
...
print(ggDo)  # This will print the plot to the PDF device
dev.off()   # Close the PDF device
...
pairwise_dfm$Significance = ifelse(pairwise_dfm$mpvalue < 0.05 & abs(pairwise_dfm$mmean) >= 0.4, 
                                  ifelse(pairwise_df$mpvalue> 0 ,colnames(assay(markersMotifs))[2],colnames(assay(markersMotifs))[1]),
                                  'Not significant')
pairwise_dfm <- na.omit(pairwise_dfm)
write.csv(pairwise_dfm,file=paste0(project_name,"_pairwise_motifs.csv"),row.names = FALSE)

volcanom<- EnhancedVolcano(pairwise_dfm,
                lab =pairwise_dfm$pairwise_motifs ,
                x = 'mmean',
                y = 'mpvalue',
                ylim=c(0,abs(min(log10(pairwise_dfm$mpvalue)))),
                xlim= c(-2.5,2.5),
                title = paste0(colnames(assay(markersMotifs))[2],' vs ',colnames(assay(markersMotifs))[1]),
                pCutoff = 0.05,
                FCcutoff = 0.4,
                pointSize = 1.0,
                labSize = 4.0)
...
print(volcanom)  # This will print the plot to the PDF device
dev.off()   # Close the PDF device
